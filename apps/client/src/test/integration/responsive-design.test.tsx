...
beforeAll(() => {
  ...
  const notifyAll = () => {
    for (const entry of registry) {
      ...
      if (next !== entry.mql.matches) {
        ...
        const listeners = (
          entry.mql as unknown as { __listeners?: Set<Listener>; __legacy?: Set<Listener> }
        ).__listeners;
        const legacy = (
          entry.mql as unknown as { __listeners?: Set<Listener>; __legacy?: Set<Listener> }
        ).__legacy;
  ...
  Object.defineProperty(window, 'matchMedia', {
    ...
    value: vi.fn().mockImplementation((query: string) => {
      ...
      (mql as unknown as { __listeners: Set<Listener>; __legacy: Set<Listener> }).__listeners =
        listeners;
  ...
  (window as unknown as { __notifyMatchMediaChange?: () => void }).__notifyMatchMediaChange =
    notifyAll;
});
