...
describe('alert history selection', () => {
	it('uses inclusive range boundaries', () => {
		const data = [
			entry('2026-01-01T00:00:00.000Z', AlertHistoryEventType.STATUS_CHANGED),
			entry('2026-01-02T00:00:00.000Z', AlertHistoryEventType.RESOLVED),
			entry('2026-01-03T00:00:00.001Z', AlertHistoryEventType.COMMENT_ADDED),
		];
		expect(filterHistoryByRange(data, range('2026-01-01T00:00:00.000Z', '2026-01-02T00:00:00.000Z'))).toHaveLength(
			2
		);
	...
	it('uses UPDATED as a fallback only when real events are outside the window', () => {
		...
		expect(selectHistoryEntries(data, range('2026-01-01T00:00:00.000Z', '2026-01-02T00:00:00.000Z'))).toEqual([
			updated,
		]);
