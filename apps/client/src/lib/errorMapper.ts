...
const FIELD_ERROR_MESSAGES: Record<string, Record<string, string>> = {
  ...
  accessKeyId: {
    ...
    invalid_regex:
      "Access Key ID must start with AKIA and be 20 characters long",
...
function mapValidationErrorDetail(detail: ValidationErrorDetail): string {
  ...
  if (
    ...
    FIELD_ERROR_MESSAGES[fieldName][detail.code]
  ) {
  ...
  if (response?.details && Array.isArray(response.details)) {
    const fieldError = response.details.find(
      (detail: ValidationErrorDetail) =>
        detail.path[detail.path.length - 1] === fieldName,
    );
...
export function isValidationError(
  ...
  return (
    ...
    (response?.error === "Validation error" ||
      response?.error?.includes("Validation error")) &&
