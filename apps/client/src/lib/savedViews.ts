...
export async function saveView(view: SavedView): Promise<void> {
  try {
    ...
    if (!response.success) {
      console.warn(
        "API save failed, falling back to localStorage",
        response.error,
      );
...
export async function getActiveViewId(): Promise<string | undefined> {
  try {
    ...
    if (response.error && response.error.includes("404")) {
      ...
      const localStorageViewId = localStorage.getItem(
        "OpsiMate-active-view-id",
      );
    ...
    console.warn(
      "API get active view failed, falling back to localStorage",
      response.error,
    );
...
export async function setActiveViewId(
  ...
  try {
    if (viewId) {
      ...
      if (!response.success) {
        console.warn(
          "API set active view failed, falling back to localStorage",
          response.error,
        );
        // Fall back to localStorage
