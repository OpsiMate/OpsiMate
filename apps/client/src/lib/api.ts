...
export const apiKeysApi = {
    ...
    createApiKey: async (apiKeyData: { name: string; expiresAt?: string }): Promise<ApiResponse<ApiKeyResponse>> => {
        try {
            const response = await apiRequest<ApiKeyResponse>('/api-keys', 'POST', apiKeyData);
            return response;
        } catch (error) {
    ...
    updateApiKey: async (
        ...
        try {
            const response = await apiRequest<ApiKey>(`/api-keys/${apiKeyId}`, 'PUT', updates);
            return response;
        } catch (error) {
    ...
    deleteApiKey: async (apiKeyId: number): Promise<ApiResponse<void>> => {
        try {
            const response = await apiRequest<void>(`/api-keys/${apiKeyId}`, 'DELETE');
            return response;
        } catch (error) {
