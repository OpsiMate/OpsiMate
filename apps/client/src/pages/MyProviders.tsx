...
export function MyProviders() {
  ...
  const [selectedServerForService, setSelectedServerForService] =
  ...
  const [selectedServiceForDrawer, setSelectedServiceForDrawer] =
  ...
  const fetchProviders = async () => {
    ...
    try {
      ...
      if (response.success && response.data && response.data.providers) {
        const apiProviders: Provider[] = response.data.providers.map(
          (provider) => {
          ...
          (acc, service) => {
            ...
            if (providerId) {
              ...
              acc[providerId].push({
                ...
                name: service.name,
                status: service.serviceStatus as
                  | "running"
                  | "stopped"
                  | "error"
                  | "unknown",
  ...
  const filteredProviders = providerInstances.filter((provider) => {
    ...
    const matchesTab =
      activeTab === "all" || getProviderCategory(type) === activeTab;
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const updatedProvider: Provider = {
          ...
          providerType:
            refreshedProvider.providerType as Provider["providerType"],
          status: services.some((s) => s.serviceStatus === "running")
            ? "running"
            : "stopped",
  ...
  const refreshProviderServices = async (provider: Provider) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const serviceConfigs: ServiceConfig[] = providerServices.map(
          (service) => ({
            id: service.id.toString(),
            name: service.name,
            status: service.serviceStatus as
  ...
  const handleServiceAction = async (
    ...
    try {
      ...
      setTimeout(
        () =>
          handleRowClick(
            providerInstances.find((i) => i.id === Number(providerId))!,
          ),
        2000,
      );
    } catch (error) {
  ...
  const updateUIAfterServiceAddition = (
    providerId: number,
    ...
    setProviderInstances((prevProviders) => {
      ...
      console.log(
        "Updated providers after service addition:",
        updatedProviders,
      );
  ...
  const handleServiceStatusChange = async (
    ...
    try {
      ...
      if (
        selectedProvider &&
        selectedProvider.id === Number(providerId) &&
  ...
  const updateUIAfterServiceDeletion = (serviceId: string) => {
    setProviderInstances((prevProviders) => {
      ...
      console.log(
        "Updated providers after service deletion:",
        updatedProviders,
      );
      return updatedProviders;
    });
        ...
        const updatedProviders = providerInstances.map((provider) => {
          if (provider.id === Number(providerId)) {
            return {
              ...
              providerType:
                updatedData.providerType as Provider["providerType"],
            };
          }
          return provider;
        });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredProviders.map((provider) => {
                ...
                const hasMoreServices =
                  hasServices && provider.services!.length > 3;
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuPortal>
                          ...
                          >
                            <DropdownMenuItem
                              onClick={() =>
                                handleRefreshProvider(String(provider.id))
                              }
                            >
      ...
      {selectedServerForService && (
        <AddServiceDialog
          ...
          onServiceAdded={(service) =>
            handleAddService(selectedServerForService.id, service)
          }
        />
      )}
