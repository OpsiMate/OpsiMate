...
export const MyProviders = () => {
  ...
  const handleServiceAction = async (
    ...
    try {
      ...
      setTimeout(
        () => handleRowClick(providerInstances.find(i => i.id === Number(providerId))!),
        2000
      );
    } catch (error) {
  ...
  const handleDeleteService = async (serviceId: string) => {
    try {
      ...
      toast({
        ...
        description:
          error instanceof Error ? error.message : 'There was a problem deleting the service',
        variant: 'destructive',
      });
    }
  };
                                      ...
                                      <DropdownMenu>
                                        ...
                                        <DropdownMenuPortal>
                                          ...
                                          >
                                            <DropdownMenuItem
                                              onClick={() => {
                                                const parentProvider = providerInstances.find(
                                                  p =>
                                                    p.services &&
                                                ...
                                                const mappedService: Service = {
                                                  ...
                                                  serviceType:
                                                    ...
                                                    'MANUAL',
                                                  createdAt:
                                                    (service as { createdAt?: string }).createdAt ||
                                                    new Date().toISOString(),
