...
const TVMode = ({
  ...
  const autoRefresh =
    searchParams.get('autoRefresh') === 'false' ? false : (propAutoRefresh ?? true);
  const refreshInterval =
    parseInt(searchParams.get('refreshInterval') || '') || propRefreshInterval || 30000; // 30 seconds
  const viewRotation =
    searchParams.get('viewRotation') === 'true' ? true : (propViewRotation ?? false); // Off by default
  const rotationInterval =
    parseInt(searchParams.get('rotationInterval') || '') || propRotationInterval || 120000; // 2 minutes
  const defaultView =
    (searchParams.get('defaultView') as 'all' | 'running' | 'stopped' | 'error') || 'all';
        ...
        {isLoading ? (
          ...
          >
            {filteredServices.map(service => {
              const isCompact =
                smartGridConfig.cardSize === 'xs' || smartGridConfig.cardSize === 'sm';
                                ...
                                <div className='space-y-2'>
                                  ...
                                  {service.serviceAlerts && service.serviceAlerts.length > 3 && (
                                    <p className='text-xs text-red-600 font-medium'>
                                      +{service.serviceAlerts.length - 3} more alerts
                                    </p>
                                  )}
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align='end' className='w-32'>
                                ...
                                <DropdownMenuItem
                                  ...
                                  disabled={
                                    startServiceMutation.isPending || stopServiceMutation.isPending
                                  }
                                  className='text-xs'
                                >
