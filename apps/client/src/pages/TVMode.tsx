...
const TVMode = ({
  ...
  const refreshInterval =
    parseInt(searchParams.get("refreshInterval") || "") ||
    propRefreshInterval ||
    30000; // 30 seconds
  const viewRotation =
    searchParams.get("viewRotation") === "true"
      ? true
      : (propViewRotation ?? false); // Off by default
  const rotationInterval =
    parseInt(searchParams.get("rotationInterval") || "") ||
    propRotationInterval ||
    120000; // 2 minutes
  const defaultView =
    (searchParams.get("defaultView") as
      | "all"
      | "running"
      | "stopped"
      | "error") || "all";
  ...
  const servicesWithAlerts = useMemo(() => {
    ...
    return services.map((service) => {
      ...
      const uniqueAlerts = serviceAlerts.filter(
        (alert, index, self) =>
  ...
  const baseFilteredServices = useMemo(() => {
    ...
    if (savedFilters.providerType && savedFilters.providerType.length > 0) {
      filtered = filtered.filter(
        (service) =>
          ...
          savedFilters.providerType!.includes(service.provider.providerType),
      );
    }

    if (
      savedFilters.containerNamespace &&
      savedFilters.containerNamespace.length > 0
    ) {
      filtered = filtered.filter(
        (service) =>
          service.containerDetails?.namespace &&
          savedFilters.containerNamespace!.includes(
            service.containerDetails.namespace,
          ),
      );
    }
    ...
    if (savedFilters.tags && savedFilters.tags.length > 0) {
      ...
      filtered = filtered.filter(
        (service) =>
          service.tags &&
          service.tags.some((tag) => savedFilters.tags!.includes(tag.name)),
      );
      console.log(
        "TV Mode - After tags filter:",
        filtered.length,
        "(was",
        beforeCount,
        ")",
  ...
  const stats = useMemo(() => {
    ...
    const totalRunning = servicesWithAlerts.filter(
      (s) => s.serviceStatus === "running",
    ).length;
    const totalStopped = servicesWithAlerts.filter(
      (s) => s.serviceStatus === "stopped",
    ).length;
    const totalError = servicesWithAlerts.filter(
      (s) => s.serviceStatus === "error",
    ).length;
    const totalUnknown = servicesWithAlerts.filter(
      (s) => s.serviceStatus === "unknown",
    ).length;
    const totalAlerts = servicesWithAlerts.reduce(
    ...
    const filteredRunning = baseFilteredServices.filter(
      (s) => s.serviceStatus === "running",
    ).length;
    const filteredStopped = baseFilteredServices.filter(
      (s) => s.serviceStatus === "stopped",
    ).length;
    const filteredError = baseFilteredServices.filter(
      (s) => s.serviceStatus === "error",
    ).length;
    const filteredUnknown = baseFilteredServices.filter(
      (s) => s.serviceStatus === "unknown",
    ).length;
      ...
      <div className="h-screen overflow-y-auto bg-background text-foreground p-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-muted-foreground space-y-0.5">
                  <p>
                    {currentTime.toLocaleString()} • Auto-refresh:{" "}
                    {autoRefresh ? "ON" : "OFF"}
                    {viewRotation && " • View Rotation: ON"}
                  </p>
                  <p>
                    Grid: {smartGridConfig.columns} cols (
                    {smartGridConfig.cardSize})
          ...
          <div className="flex flex-wrap items-center justify-center gap-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex flex-wrap gap-1">
                {(["all", "running", "stopped", "error"] as const).map(
                  (state, index) => {
        ...
        {isLoading ? (
          ...
          <div
            ...
            style={{
              gridTemplateColumns: `repeat(${smartGridConfig.columns}, minmax(0, 1fr))`,
            }}
          >
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className={cn("space-y-1", isCompact && "space-y-0.5")}
                    >
                      <h3
                        className={cn(
