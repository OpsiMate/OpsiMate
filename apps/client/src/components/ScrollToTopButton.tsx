...
const ScrollToTopButton: React.FC<{ right?: string; bottom?: string }> = ({ right = '22px', bottom = '22px' }) => {
	...
	const button = (
		<button
			...
			className="fixed z-999999 p-3 rounded-full shadow-lg focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-slate-400 transition-all duration-200 transform hover:scale-105 active:scale-95 bg-slate-800/80 text-white"
		>
			<ArrowUp size={18} />
		</button>
	);
