  ...
  const facets = useMemo(() => {
    ...
    services.forEach((service) => {
      if (service.serviceStatus) {
        ...
        newFacets.serviceStatus[value] =
          (newFacets.serviceStatus[value] || 0) + 1;
      }
      ...
      if (service.provider?.providerType) {
        ...
        newFacets.providerType[value] =
          (newFacets.providerType[value] || 0) + 1;
      }

      if (service.provider?.name) {
        const value = String(service.provider.name);
        newFacets.providerName[value] =
          (newFacets.providerName[value] || 0) + 1;
      }

      if (service.containerDetails?.namespace) {
        const value = String(service.containerDetails.namespace);
        newFacets.containerNamespace[value] =
          (newFacets.containerNamespace[value] || 0) + 1;
      }
