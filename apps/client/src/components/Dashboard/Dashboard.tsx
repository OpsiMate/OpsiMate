...
export const Dashboard = () => {
  ...
  useEffect(() => {
    if (customFields.length > 0) {
      ...
      setColumnOrder((prev) => {
        const customFieldKeys = customFields.map(
  ...
  useEffect(() => {
    if (selectedService) {
      const updatedService = servicesWithAlerts.find(
          ...
          switch (key) {
            ...
            case "providerType":
              ...
              return filterValues.includes(
                String(service.provider?.providerType.toLowerCase()),
              );

            case "providerName":
              // Nested provider property
              return filterValues.includes(
                String(service.provider?.name.toLowerCase()),
              );

            case "containerNamespace":
              // Nested container details property
              return filterValues.includes(
                String(service.containerDetails?.namespace.toLowerCase()),
              );
            ...
            default:
              ...
              return filterValues.includes(
                String(service[key as keyof Service]).toLowerCase(),
              );
          }
        }
        return true;
  ...
  const handleServicesSelect = (services: Service[]) => {
    ...
    if (services.length === 1) {
      ...
      const serviceWithAlerts = servicesWithAlerts.find(
        (s) => s.id === services[0].id,
      );
          ...
          <div className="flex flex-row h-full">
            ...
            >
              ...
              <Button
                ...
                title={
                  filterPanelCollapsed ? "Expand filters" : "Collapse filters"
                }
              >
                {filterPanelCollapsed ? (
