...
function SortableHeader({
  ...
  return (
    <TableHead
      className={cn(
        "font-medium cursor-pointer hover:bg-muted/50 transition-colors select-none",
        className,
      )}
      onClick={() => onSort(field)}
    >
    ...
    if (sortConfig) {
      filtered = [...filtered].sort((a, b) => {
        ...
        switch (sortConfig.field) {
          ...
          case "serviceIP":
            aValue =
              (a.serviceType === "SYSTEMD"
                ? a.provider.providerIP
                : a.serviceIP) || "";
            bValue =
              (b.serviceType === "SYSTEMD"
                ? b.provider.providerIP
                : b.serviceIP) || "";
            break;
          case "serviceStatus":
          ...
          case "containerDetails":
            aValue =
              a.serviceType === "DOCKER"
                ? a.containerDetails?.image || ""
                : a.serviceType;
            bValue =
              b.serviceType === "DOCKER"
                ? b.containerDetails?.image || ""
                : b.serviceType;
            break;
          case "tags":
            aValue =
              a.tags && a.tags.length > 0 ? a.tags[0].name.toLowerCase() : "";
            bValue =
              b.tags && b.tags.length > 0 ? b.tags[0].name.toLowerCase() : "";
            break;
          case "alerts":
            ...
            <TableHeader className="sticky top-0 bg-card z-10">
              <TableRow className="hover:bg-transparent">
                ...
                {visibleColumns.serviceIP && (
                  <TableHead className="font-medium">Service IP</TableHead>
                )}
                        ...
                        default:
                          ...
                          if (columnId.startsWith("custom-")) {
                            const fieldId = parseInt(
                              columnId.replace("custom-", ""),
                            );
                            const fieldValue =
                              service.customFields?.[fieldId] || "";
                            return (
                              <TableCell
                                key={columnId}
                                className="py-1 px-2 text-sm"
