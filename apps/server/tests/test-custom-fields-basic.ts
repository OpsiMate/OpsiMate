...
async function testCustomFieldsBasic() {
  ...
  try {
    ...
    console.log(
      `✅ Test service created with ID: ${serviceResult.lastInsertRowid}`,
    );
    ...
    await valueRepo.upsertCustomFieldValue(
      serviceResult.lastInsertRowid as number,
      result.lastID,
      "test value",
    );
    ...
    const values = await valueRepo.getCustomFieldValuesByServiceId(
      serviceResult.lastInsertRowid as number,
    );
