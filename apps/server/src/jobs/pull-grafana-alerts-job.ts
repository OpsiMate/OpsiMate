...
export class PullGrafanaAlertsJob {
  ...
  startPullGrafanaAlertsJob = () => {
    logger.info(
      "[Job] Starting refreshAllProvidersServices job (every 10 minutes)",
    );
  ...
  private async pullGrafanaAlerts() {
    const grafanaIntegration = await this.integrationBL.getIntegrationByType(
    ...
    if (!token) {
      logger.warn(
        `No token for Grafana integration ${grafanaIntegration.name}`,
      );
      return;
    }
      ...
      try {
        await this.alertBL.insertOrUpdateAlert({
          ...
          starts_at: alert.startsAt
            ? new Date(alert.startsAt).toISOString()
            : "",
    ...
    const resolvedAlerts = await this.alertBL.deleteAlertsNotInIds(
