...
function getSshConfig(provider: Provider) {
  const { providerIP, username, privateKeyFilename, password, SSHPort } =
    provider;

  // Ensure at least one authentication method is provided
  if (!privateKeyFilename && !password) {
    throw new Error(
      "Either privateKeyFilename or password must be provided for SSH authentication",
    );
  }
  ...
  if (privateKeyFilename) {
    const encryptedKey = fs.readFileSync(
      getKeyPath(privateKeyFilename),
      "utf-8",
    );
 ...
 */
async function execCommandWithAutoSudo(
  ssh: NodeSSH,
  command: string,
  ...
  if (
    result.code !== 0 &&
    isPermissionError(result.stderr) &&
    getVmConfig().try_with_sudo
  ) {
...
export async function connectAndListContainers(
  provider: Provider,
  ...
  return result.stdout
    ...
    .map((line) => {
      ...
      return {
        name: name,
        serviceStatus: status.toLowerCase().includes("up")
          ? "running"
          : "stopped",
...
export async function getServiceLogs(
  ...
  try {
    ...
    return logs.length > 0
      ? logs
      : ["No error logs found in the last 24 hours"];
  } catch (error) {
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error occurred";
    throw new Error(
      `Failed to get logs for service ${serviceName}: ${errorMessage}`,
    );
  } finally {
    ssh.dispose();
  }
...
export async function getSystemServiceLogs(
  ...
  try {
    ...
    if (result.code !== 0) {
      throw new Error(
        `Failed to get logs for ${serviceName}: ${result.stderr}`,
      );
    }
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error occurred";
    throw new Error(
      `Failed to get logs for system service ${serviceName}: ${errorMessage}`,
    );
  } finally {
    ssh.dispose();
  }
...
export async function testConnection(
  ...
  try {
    ...
    const success =
      result.code === 0 && result.stdout.trim() === "Connection test";
    if (!success) {
    ...
    logger.error(
      `Connection test failed for provider ${provider.providerIP}:`,
      error,
    );
    ...
    if (
      ...
      return {
        success: false,
        error: "Authentication failed - invalid credentials or SSH key",
      };
    } else if (errorMessage.includes("ECONNREFUSED")) {
      return {
        success: false,
        error:
          "Connection refused - server may be down or SSH port is incorrect",
      };
    } else if (
      ...
      return {
        success: false,
        error: "Host not found - check the IP address or hostname",
      };
...
export async function checkSystemServiceStatus(
  ...
  try {
    ...
    const isActiveResult = await execCommandWithAutoSudo(
      ssh,
      `systemctl is-active ${serviceName}`,
    );
    if (isActiveResult.code !== 0) {
      throw new Error(
        `Failed check system service status ${serviceName}: ${isActiveResult.stderr}`,
      );
    }
    ...
    const statusResult = await execCommandWithAutoSudo(
      ssh,
      `systemctl status ${serviceName} --no-pager -l`,
    );
    if (statusResult.code !== 0) {
      throw new Error(
        `Failed check system service status ${serviceName}: ${statusResult.stderr}`,
      );
    }
    ...
    if (
      statusOutput.includes("active (running)") ||
      statusOutput.includes("active (exited)")
    ) {
      return "running";
    }
