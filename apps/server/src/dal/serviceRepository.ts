...
export class ServiceRepository {
  ...
  async getServicesWithProvider(): Promise<ServiceWithProvider[]> {
    return runAsync(() => {
      ...
      const rows: ServiceRowWithProviderRow[] = this.db
        .prepare(query)
        .all() as ServiceRowWithProviderRow[];
  ...
  async getServiceWithProvider(
    ...
    return runAsync(() => {
      ...
      const row: ServiceRowWithProviderRow = this.db
        .prepare(query)
        .get(id) as ServiceRowWithProviderRow;
      if (!row) return null;
