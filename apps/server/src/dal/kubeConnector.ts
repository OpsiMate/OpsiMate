...
const createClient = (_provider: Provider): k8s.CoreV1Api => {
  const encryptedKubeConfig = fs.readFileSync(
    getKeyPath(_provider.privateKeyFilename!),
    "utf-8",
  );
...
async function getServicePodLogs(
  ...
  // Get the Service
  const service = await coreV1.readNamespacedService({
    name: serviceName,
    namespace,
  });
  ...
  for (const pod of pods) {
    ...
    const logText = await coreV1.readNamespacedPodLog({
      name: podName,
      namespace,
    });
    logs.push(logText || "");
...
const restartK8RServicePods = async (
  provider: Provider,
  serviceName: string,
) => {
  ...
  const labelSelector = Object.entries(selector)
    .map(([k, v]) => `${k}=${v}`)
    .join(",");
  const podsList = await k8sApi.listNamespacedPod({
    namespace: "default",
    labelSelector,
  });
  const pods: k8s.V1Pod[] = podsList.items ?? [];
    ...
    .map(async (service: k8s.V1Service) => {
      ...
      if (selector && Object.keys(selector).length > 0) {
        ...
        if (items.length === 0) {
          ...
          serviceStatus = [
            ...new Set(
              items.map((i: k8s.V1Pod) => i.status?.phase || "Unknown"),
            ),
          ].join(", ");
        }
      }
