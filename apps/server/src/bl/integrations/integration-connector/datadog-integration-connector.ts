...
} from "../../../dal/external-client/datadog-client";
  ...
  async getUrls(
    ...
    try {
      ...
      if (
        ...
        this.logger.error(
          "Missing Datadog API key or Application key in credentials",
        );
        return [];
      }
      ...
      this.logger.info(
        `Found ${dashboards.length} Datadog dashboards matching tags: ${tags.join(", ")}`,
      );
