...
import {
  DashboardResult,
  KibanaClient,
} from "../../../dal/external-client/kibana-client";
...
export class KibanaIntegrationConnector implements IntegrationConnector {
  ...
  async getUrls(
    ...
    try {
      ...
      const dashboardPromises: Promise<IntegrationUrls[]>[] = tags.map(
        async (tag: string) => {
          try {
            const dashboards = await kibanaClient.searchDashboardsByTag(tag);
            this.logger.info(
              `Found ${dashboards.length} dashboards with tag '${tag}'`,
            );
            ...
            this.logger.error(
              `Error fetching Kibana dashboards for tag ${tag}:`,
              error,
            );
