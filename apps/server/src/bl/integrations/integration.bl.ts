...
export class IntegrationBL {
  ...
  async createIntegration(
    ...
    try {
      logger.info(
        `Starting to create integration: ${JSON.stringify(integrationToCreate)}`,
      );
      const { lastID } =
        await this.integrationRepo.createIntegration(integrationToCreate);
      logger.info(`Integration created with ID: ${lastID}`);

      const createdIntegration =
        await this.integrationRepo.getIntegrationById(lastID);
      logger.info(
        `Fetched created integration: ${JSON.stringify(createdIntegration)}`,
      );
  ...
  async updateIntegration(
    ...
    try {
      await this.integrationRepo.updateIntegration(
        integrationId,
        integrationToUpdate,
      );
  ...
  private async validateIntegrationExists(
    integrationId: number,
  ): Promise<void> {
    const integration =
      await this.integrationRepo.getIntegrationById(integrationId);
    if (!integration) {
  ...
  async getIntegrationUrls(integrationId: number, tags: string[]) {
    ...
    return await integrationConnectorFactory(integration.type).getUrls(
      integration,
      tags,
    );
  }
