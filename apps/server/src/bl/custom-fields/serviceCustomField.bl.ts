...
export class ServiceCustomFieldBL {
  ...
  async createCustomField(name: string): Promise<number> {
    try {
      ...
      const duplicate = existingField.find(
      ...
      const result = await this.customFieldRepository.createCustomField({
        name,
      });
      logger.info(
        `Successfully created custom field '${name}' with ID: ${result.lastID}`,
      );
  ...
  async updateCustomField(id: number, name: string): Promise<boolean> {
    try {
      ...
      const existingField =
        await this.customFieldRepository.getCustomFieldById(id);
      if (!existingField) {
      ...
      const updated = await this.customFieldRepository.updateCustomField(id, {
        name,
      });

      if (updated) {
        logger.info(
          `Successfully updated custom field ${id} to name '${name}'`,
        );
      } else {
  ...
  async deleteCustomField(id: number): Promise<boolean> {
    try {
      ...
      const deletedValuesCount =
        await this.customFieldValueRepository.deleteAllValuesForCustomField(id);
      logger.info(
        `Deleted ${deletedValuesCount} custom field values for field ${id}`,
      );
      ...
      if (deleted) {
        logger.info(
          `Successfully deleted custom field '${field.name}' and all its values`,
        );
      }
  ...
  async upsertCustomFieldValue(
    ...
    try {
      logger.info(
        `Upserting custom field value for service ${serviceId}, field ${customFieldId}`,
      );

      // Validate that the custom field exists
      const field =
        await this.customFieldRepository.getCustomFieldById(customFieldId);
      if (!field) {
        throw new Error(`Custom field with ID ${customFieldId} not found`);
      }

      await this.customFieldValueRepository.upsertCustomFieldValue(
        serviceId,
        customFieldId,
        value,
      );
      logger.info(
        `Successfully upserted custom field value for service ${serviceId}, field ${customFieldId}`,
      );
    } catch (error) {
      logger.error(
        `Error upserting custom field value for service ${serviceId}, field ${customFieldId}`,
        error,
      );
      throw error;
    }
  ...
  async getCustomFieldValuesForService(
    serviceId: number,
  ): Promise<ServiceCustomFieldValue[]> {
    try {
      logger.info(`Fetching custom field values for service ${serviceId}`);
      const values =
        await this.customFieldValueRepository.getCustomFieldValuesByServiceId(
          serviceId,
        );
      logger.info(
        `Successfully fetched ${values.length} custom field values for service ${serviceId}`,
      );

      return values;
    } catch (error) {
      logger.error(
        `Error fetching custom field values for service ${serviceId}`,
        error,
      );
      throw error;
    }
  ...
  async getCustomFieldValue(
    serviceId: number,
    customFieldId: number,
  ): Promise<ServiceCustomFieldValue | null> {
    try {
      logger.info(
        `Fetching custom field value for service ${serviceId}, field ${customFieldId}`,
      );
      const value = await this.customFieldValueRepository.getCustomFieldValue(
        serviceId,
        customFieldId,
      );
      ...
      if (value) {
        logger.info(
          `Successfully fetched custom field value for service ${serviceId}, field ${customFieldId}`,
        );
      } else {
        logger.info(
          `No custom field value found for service ${serviceId}, field ${customFieldId}`,
        );
      }
      ...
      logger.error(
        `Error fetching custom field value for service ${serviceId}, field ${customFieldId}`,
        error,
      );
      throw error;
    }
  ...
  async deleteCustomFieldValue(
    ...
    try {
      logger.info(
        `Deleting custom field value for service ${serviceId}, field ${customFieldId}`,
      );
      const deleted =
        await this.customFieldValueRepository.deleteCustomFieldValue(
          serviceId,
          customFieldId,
        );
      ...
      if (deleted) {
        logger.info(
          `Successfully deleted custom field value for service ${serviceId}, field ${customFieldId}`,
        );
      } else {
        logger.warn(
          `No custom field value found to delete for service ${serviceId}, field ${customFieldId}`,
        );
      }
      ...
      logger.error(
        `Error deleting custom field value for service ${serviceId}, field ${customFieldId}`,
        error,
      );
      throw error;
    }
  ...
  // Helper methods
  async getCustomFieldsWithValuesForService(
    serviceId: number,
    ...
    try {
      ...
      logger.error(
        `Error getting custom fields with values for service ${serviceId}`,
        error,
      );
      throw error;
    }
  ...
  async deleteAllValuesForService(serviceId: number): Promise<number> {
    try {
      ...
      const deletedCount =
        await this.customFieldValueRepository.deleteAllValuesForService(
          serviceId,
        );
      logger.info(
        `Deleted ${deletedCount} custom field values for service ${serviceId}`,
      );

      return deletedCount;
    } catch (error) {
      logger.error(
        `Error deleting custom field values for service ${serviceId}`,
        error,
      );
      throw error;
    }
