...
export class ServicesBL {
  ...
  async createService(
    serviceToCreate: Omit<Service, "id" | "createdAt">,
    user: User,
  ): Promise<Service> {
    try {
      logger.info(`Starting to create service`, {
        extraArgs: { ...serviceToCreate },
      });
      ...
      if (!createdService) {
        logger.error(
          `Failed to fetch service immediately after creation with ID: ${lastID}`,
        );
