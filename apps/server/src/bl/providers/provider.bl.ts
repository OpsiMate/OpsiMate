...
export class ProviderBL {
  ...
  async createProvider(
    providerToCreate: Omit<Provider, 'id'>,
    user: User
  ): Promise<Provider> {
    try {
      logger.info(`Starting to create provider`, {
        extraArgs: { ...providerToCreate },
      });

      // Resolve secretId to privateKeyFilename if provided
      const resolvedProvider = { ...providerToCreate };
      if (providerToCreate.secretId) {
        const secret = await this.secretsMetadataRepo.getSecretById(
          providerToCreate.secretId
        );
        if (!secret) {
          throw new Error(
            `Secret with ID ${providerToCreate.secretId} not found`
          );
        }
      ...
      const { lastID } =
        await this.providerRepo.createProvider(resolvedProvider);
      logger.info(`Provider created with ID: ${lastID}`);

      const createdProvider = await this.providerRepo.getProviderById(lastID);
      logger.info(`Fetched created provider`, {
        extraArgs: { ...createdProvider },
      });

      this.auditBL.logAction({
  ...
  async updateProvider(
    ...
    try {
      ...
      if (providerToUpdate.secretId) {
        const secret = await this.secretsMetadataRepo.getSecretById(
          providerToUpdate.secretId
        );
        if (!secret) {
          throw new Error(
            `Secret with ID ${providerToUpdate.secretId} not found`
          );
        }
        resolvedProvider.privateKeyFilename = secret.fileName;
        delete resolvedProvider.secretId;
  ...
  async discoverServicesInProvider(
    providerId: number
  ...
  async addServicesToProvider(
    ...
    logger.info(
      `Starting to add services [${services.length}] to provider: ${providerId}`
    );
    await this.validateProviderExists(providerId);
  ...
  async refreshProvider(
    providerId: number
  ): Promise<{ provider: Provider; services: Service[] }> {
    ...
    try {
      ...
      const updatedServices =
        await this.serviceRepo.getServicesByProviderId(providerId);
      ...
      throw new Error(
        `Failed to refresh provider: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async refreshProviderServices(provider: Provider): Promise<void> {
    ...
    const discoveredServices: DiscoveredService[] =
      await connector.discoverServices(provider);
    const dbServices = await this.serviceRepo.getServicesByProviderId(
      provider.id
    );
    ...
    for (const dbService of dbServices) {
      if (dbService.serviceType === ServiceType.SYSTEMD) {
        try {
          const actualStatus = await checkSystemServiceStatus(
            provider,
            dbService.name
          );
          ...
          if (actualStatus !== dbService.serviceStatus) {
            ...
            logger.info(
              `Updated systemd service ${dbService.name} status to ${actualStatus}`
            );
          }
        } catch (error) {
          logger.error(
            `Failed to check systemd service ${dbService.name} status:`,
            error
          );
        }
      } else {
        const matchedService = this.findMatchingService(
          discoveredServices,
          dbService.name
        );
        ...
        if (
          matchedService &&
          matchedService.serviceStatus !== dbService.serviceStatus
        ) {
          await this.serviceRepo.updateService(dbService.id, {
  ...
  private findMatchingService(
    ...
    return discoveredServices.find(
      (ds) =>
        ds.name.trim().toLowerCase() === dbServiceName.trim().toLowerCase()
    );
  }
}
