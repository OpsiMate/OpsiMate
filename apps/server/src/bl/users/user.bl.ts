...
export class UserBL {
    ...
    async register(email: string, fullName: string, password: string): Promise<User> {
        ...
        if (userCount > 0) throw new Error('Registration is disabled after first admin');
        ...
        void this.auditBL.logAction({ // registered by self
    ...
    async updateProfile(id: number, fullName: string, newPassword?: string): Promise<User> {
        ...
        void this.auditBL.logAction({ // updated by self
            actionType: AuditActionType.DELETE,
            resourceType: AuditResourceType.USER,
