...
export class UsersController {
  ...
  registerHandler = async (req: Request, res: Response) => {
    try {
      ...
      if (isZodError(error)) {
        ...
        error.message === "Registration is disabled after first admin"
      ) {
  ...
  createUserHandler = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      const { email, fullName, password, role } = CreateUserSchema.parse(
        req.body,
      );
      const result = await this.userBL.createUser(
        email,
        fullName,
        password,
        role,
  ...
  updateProfileHandler = async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      const updatedUser = await this.userBL.updateProfile(
        req.user.id,
        fullName,
        newPassword,
      );

      const responseData: { user: User; token?: string | undefined } = {
        user: updatedUser,
      };
  ...
  updateUserPasswordHandler = async (
    ...
    try {
      ...
      if (!fullName && !email && !role) {
        return res.status(400).json({
          success: false,
          error:
            "At least one field (fullName, email, or role) must be provided",
        });
      }
      ...
      const updatedUser = await this.userBL.updateUser(userId, {
        fullName,
        email,
        role,
      });
