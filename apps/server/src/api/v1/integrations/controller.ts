...
export class IntegrationController {
  ...
  getIntegrations = async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        success: true,
        data: { integrations: integrations.map(toIntegrationResponse) },
      });
    } catch (error) {
  ...
  createIntegration = async (req: Request, res: Response) => {
    try {
      ...
      const createdIntegration: Integration =
        await this.integrationBL.createIntegration(integrationToCreate);
  ...
  updateIntegration = async (req: Request, res: Response) => {
    try {
      ...
      if (isNaN(integrationId)) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid integration ID" });
      }

      const validatedData = CreateIntegrationSchema.parse(req.body);
      const updatedIntegration = await this.integrationBL.updateIntegration(
        integrationId,
        validatedData,
      );
  ...
  deleteIntegration = async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        success: true,
        message: "Integration and associated services deleted successfully",
      });
    } catch (error) {
  ...
  getIntegrationUrls = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const tags: string[] = Array.isArray(parsed.tags)
        ? parsed.tags
        : [parsed.tags];
      const response = await this.integrationBL.getIntegrationUrls(
        integrationId,
        tags,
      );
