...
const UpsertCustomFieldValueSchema = z.object({
  ...
  customFieldId: z
    .number()
    .int()
    .positive("Custom field ID must be a positive integer"),
...
export class CustomFieldsController {
  ...
  getCustomFieldById = async (req: Request, res: Response) => {
    try {
      ...
      const customField =
        await this.customFieldsBL.getCustomFieldById(customFieldId);
      if (customField) {
        return res.json({
          success: true,
          data: { customField },
  ...
  updateCustomField = async (req: Request, res: Response) => {
    try {
      ...
      const updated = await this.customFieldsBL.updateCustomField(
        customFieldId,
        name,
      );
  ...
  deleteCustomField = async (req: Request, res: Response) => {
    try {
      ...
      const deleted =
        await this.customFieldsBL.deleteCustomField(customFieldId);
      if (deleted) {
        return res.json({
          success: true,
  ...
  upsertCustomFieldValue = async (req: Request, res: Response) => {
    try {
      const { serviceId, customFieldId, value } =
        UpsertCustomFieldValueSchema.parse(req.body);
      await this.customFieldsBL.upsertCustomFieldValue(
        serviceId,
        customFieldId,
        value,
      );
  ...
  getCustomFieldValuesForService = async (req: Request, res: Response) => {
    try {
      ...
      const values =
        await this.customFieldsBL.getCustomFieldValuesForService(serviceId);
      return res.json({
        success: true,
        data: { values },
      });
  ...
  deleteCustomFieldValue = async (req: Request, res: Response) => {
    try {
      ...
      const deleted = await this.customFieldsBL.deleteCustomFieldValue(
        serviceId,
        customFieldId,
      );
      if (deleted) {
