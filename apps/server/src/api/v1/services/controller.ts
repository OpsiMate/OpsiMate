...
export class ServiceController {
  ...
  private async enrichServicesWithCustomFields(
    ...
    try {
      ...
      const customFieldValues: {
        [serviceId: number]: { [customFieldId: number]: string };
      } = {};

      for (const serviceId of serviceIds) {
        const values =
          await this.customFieldBL.getCustomFieldValuesForService(serviceId);
  ...
  createServiceHandler = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const service = await this.servicesBL.createService(
        validatedData,
        req.user,
      );

      const provider = await this.providerRepo.getProviderById(
      ...
      if (service.serviceType === ServiceType.SYSTEMD) {
        try {
          ...
          await this.serviceRepo.updateService(service.id, {
            serviceStatus: actualStatus,
          });
          // Update the service object for the response
          service.serviceStatus = actualStatus;
          logger.info(
            `Updated systemd service ${service.name} status to ${actualStatus}`,
          );
        } catch (error) {
      ...
      const enrichedServices = await this.enrichServicesWithCustomFields([
        { ...service, provider },
      ]);
      ...
      if (isZodError(error)) {
        ...
        return res
          ...
          .json({
            success: false,
            error: `Provider with ID ${error.provider} not found`,
          });
        ...
        return res
          ...
          .json({
            success: false,
            error: `Service with ID ${error.serviceId} not found`,
          });
      } else {
  ...
  getAllServicesHandler = async (_req: Request, res: Response) => {
    try {
      ...
      const enrichedServices =
        await this.enrichServicesWithCustomFields(services);
  ...
  getServiceByIdHandler = async (req: Request, res: Response) => {
    try {
      const { serviceId } = ServiceIdSchema.parse({
  ...
  updateServiceHandler = async (req: Request, res: Response) => {
    try {
      ...
      const updatedService =
        await this.serviceRepo.getServiceWithProvider(serviceId);
      ...
      if (this.customFieldBL) {
        try {
          const deletedValuesCount =
            await this.customFieldBL.deleteAllValuesForService(serviceId);
          logger.info(
            `Deleted ${deletedValuesCount} custom field values for service ${serviceId}`,
          );
        } catch (error) {
          logger.warn(
  ...
  startServiceHandler = async (req: Request, res: Response) => {
    try {
      ...
      await providerConnector.startService(
        provider,
        service.name,
        service.serviceType,
      );
      await this.serviceRepo.updateService(serviceId, {
  ...
  stopServiceHandler = async (req: Request, res: Response) => {
    try {
      ...
      const providerConnector = providerConnectorFactory(provider.providerType);
      await providerConnector.stopService(
        provider,
        service.name,
        service.serviceType,
      );
  ...
  getServicePodsHandler = async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        success: true,
        data: pods,
        message: "Service pods retrieved successfully",
      });
    } catch (error) {
      if (isZodError(error)) {
        return res
