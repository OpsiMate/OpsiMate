...
export class ProviderController {
  ...
  async refreshProvider(req: Request, res: Response) {
    try {
      ...
      if (isNaN(providerId)) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid provider ID" });
      }
      ...
      if (error instanceof ProviderNotFound) {
        return res
          ...
          .json({
            success: false,
            error: `Provider ${error.provider} not found`,
          });
      } else {
        ...
        return res
          ...
          .json({
            success: false,
            error: "Failed to refresh provider",
            details: message,
          });
      }
    }
  ...
  async testConnection(req: Request, res: Response) {
    try {
      ...
      if (providerData.secretId) {
        const secret = await this.secretsRepo.getSecretById(
          providerData.secretId,
        );
        if (!secret) {
          return res
            .status(400)
            .json({
              success: false,
              error: `Secret with ID ${providerData.secretId} not found`,
            });
        }
      ...
      const providerConnector = providerConnectorFactory(
        resolvedProvider.providerType,
      );
      const testResult =
        await providerConnector.testConnection(resolvedProvider);

      if (testResult.success) {
        return res
          .status(200)
          .json({ success: true, data: { isValidConnection: true } });
      } else {
        return res
          .status(200)
          .json({
  ...
  async updateProvider(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const updatedProvider = await this.providerBL.updateProvider(
        providerId,
        validatedData,
        req.user as User,
      );
  ...
  async bulkAddServices(req: Request, res: Response) {
    try {
      ...
      const newServices = await this.providerBL.addServicesToProvider(
        providerId,
        validatedData,
      );
  ...
  async discoverServices(req: Request, res: Response) {
    try {
      ...
      const discoversServices =
        await this.providerBL.discoverServicesInProvider(providerId);
