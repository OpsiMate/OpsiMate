...
export class SecretsController {
  ...
  createSecret = async (req: Request, res: Response) => {
    try {
      ...
      const { displayName, secretType } = CreateSecretsMetadataSchema.parse(
        req.body,
      );
      ...
      return res
        .status(201)
        .json({ success: true, data: { id: createdSecretId } });
    } catch (error) {
      if (isZodError(error)) {
        return res
          .status(400)
  ...
  updateSecret = async (req: Request, res: Response) => {
    try {
      ...
      if (isNaN(secretId)) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid secret ID" });
      }

      // Parse the request body for metadata updates
      const { displayName, secretType } = UpdateSecretsMetadataSchema.parse(
        req.body,
      );
