...
export const CreateProviderSchema = z.object({
  ...
  providerIP: z.string().min(1, "Hostname/IP is required").refine((value: string) => {
    return isValidHostnameOrIP(value);
  }, {
    message: "Must be a valid IP address or hostname"
  }).optional(),
  username: z.string().min(1, 'Username is required').refine((value: string) => {
    return !/\s/.test(value);
  }, {
    message: "Username cannot contain whitespace"
  }).optional(),
  secretId: z.number().optional(),
  password: z.string().min(1, 'Password is required').refine((value: string) => {
    return !/\s/.test(value);
  }, {
...
}).refine((data: any) => data.secretId || data.password, {
...
export const AddBulkServiceSchema = z.array(
  z.object({
    ...
    serviceIP: z.string().min(1, "Hostname/IP is required").refine((value: string) => {
      return isValidHostnameOrIP(value);
    }, {
      message: "Must be a valid IP address or hostname"
    }).optional(),
...
export const ProviderIdSchema = z.object({
  providerId: z.string().transform((val: string) => {
...
export const ServiceIdSchema = z.object({
  serviceId: z.string().transform((val: string) => {
    const parsed = parseInt(val);
    if (isNaN(parsed)) {
...
export const TagIdSchema = z.object({
  tagId: z.string().transform((val: string) => {
    const parsed = parseInt(val);
    if (isNaN(parsed)) {
